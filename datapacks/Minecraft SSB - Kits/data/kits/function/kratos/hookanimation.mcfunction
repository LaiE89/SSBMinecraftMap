execute at @s rotated ~ 0 positioned ^-0.2 ^2.35 ^0.27 facing entity @p[tag=Kratos] eyes positioned ~ ~-0.5 ~ run function kits:kratos/slashchain
particle flame ^-0.2 ^1.85 ^0.27 0 0 0 0 1 force
execute unless entity @s[scores={kits.timer=2..},tag=!kratoshookpulling] as @e[distance=..2,type=!#kits:non_entity,tag=!Invincible,tag=!Kratos,tag=!InLabyrinth,tag=!KratosHookDmg,limit=1,sort=nearest] at @s run function kits:kratos/hookdamage

execute if entity @s[tag=kratoshookpulling] run function kits:kratos/hookpullenemy
...
execute if entity @s[scores={kits.timer=10}] run function kits:kratos/hookpull
...
execute positioned ~ ~1 ~ if entity @p[tag=KratosPull,distance=..3] run function kits:kratos/hookend
