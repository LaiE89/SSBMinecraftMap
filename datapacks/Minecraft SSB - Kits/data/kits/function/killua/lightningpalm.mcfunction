...
execute as @e[distance=..10,limit=1,sort=nearest,type=!#kits:non_entity,tag=!Invincible,tag=!Killua,tag=!InLabyrinth] at @s run function kits:killua/lightningpalmdamage
scoreboard players set @s kits.raycastTick2 0
execute at @s positioned ~ ~ ~ rotated ~ 0 run function kits:killua/killuapassive4
