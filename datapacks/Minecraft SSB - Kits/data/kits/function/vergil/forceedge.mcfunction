...
execute at @s[scores={kits.timer=..5}] positioned ~ ~1.5 ~ run function kits:vergil/vergilpassive5
execute at @s[scores={kits.timer=..5}] run execute unless block ~ ~ ~ #kits:passable run function kits:vergil/forceedgeup
execute as @e[distance=..2.5,type=!#kits:non_entity,tag=!Invincible,tag=!Vergil,tag=!InLabyrinth] at @s run function kits:vergil/forceedgedamage
execute as @s[scores={kits.timer=7..}] if entity @p[tag=Vergil,distance=..2.5] run function kits:vergil/forceedgepickup
