...
$execute if entity @s[scores={kits.z=-$(RotMaxRange)..$(RotMaxRange)}] run function kits:arcueid/chain_vfx/end
$execute unless entity @s[scores={kits.z=-$(RotMaxRange)..$(RotMaxRange)}] at @s run function kits:arcueid/chain_vfx/slash with storage minecraft:vfx EncirclingSlash