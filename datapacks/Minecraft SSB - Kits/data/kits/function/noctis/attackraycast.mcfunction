...
execute as @s[scores={kits.raycastTick2=2222},predicate=kits:items/noctis/tall] positioned ~ ~ ~ run execute as @e[distance=..2,type=!#kits:non_entity,tag=!Invincible,tag=!Noctis,tag=!InLabyrinth,limit=1,sort=nearest] at @s run function kits:noctis/damagecalc


execute as @s[scores={kits.raycastTick2=..8}] positioned ^ ^ ^0.5 run function kits:noctis/attackraycast
