...
execute as @e[dx=0,type=!#kits:non_entity,tag=!Invincible,tag=!Noctis,tag=!InLabyrinth,limit=1,sort=nearest] run execute as @e[type=armor_stand,tag=NoctisThrownWeapon,sort=nearest,limit=1] at @s run function kits:noctis/throwlanded

execute if score @s kits.raycastTick matches 1.. positioned ^ ^ ^0.5 run function kits:noctis/projectile/raycast
