...
execute as @s[scores={kits.raycastTick=2222..}] positioned ^ ^ ^ run execute as @e[distance=..2,limit=1,sort=nearest,type=!#kits:non_entity,tag=!Invincible,tag=!Denji,tag=!InLabyrinth] at @s run function kits:chainsawman/groinkickdamage
...
execute as @s[scores={kits.raycastTick=..8}] positioned ^ ^ ^0.5 run function kits:chainsawman/chainsawmanpassive2
