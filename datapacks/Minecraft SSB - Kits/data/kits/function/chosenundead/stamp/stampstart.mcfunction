...
execute as @p[tag=ChosenUndeadStamp] at @s run function kits:chosenundead/usestamina {stamina: 50}
summon minecraft:area_effect_cloud ~ ~ ~ {Tags:["ChosenUndeadStampSlash"],Particle:{type:"block",block_state:"minecraft:air"},Radius:0f,WaitTime:0,Duration:12}
...
# execute as @p[tag=ChosenUndeadStamp] at @s rotated ~ 0 run function kits:chosenundead/stamp/stampraycast
