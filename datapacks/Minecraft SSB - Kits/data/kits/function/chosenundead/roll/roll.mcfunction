...
schedule clear kits:chosenundead/stamp/stampstart
tag @s add Invincible
execute as @s at @s run tp @s ~ ~ ~ ~ 90
function kits:chosenundead/usestamina {stamina: 20}
particle block{block_state:"minecraft:gravel"} ~ ~0.25 ~ 0.5 0 0.5 0 40 force

execute at @s run function kits:chosenundead/roll/rolldirection
