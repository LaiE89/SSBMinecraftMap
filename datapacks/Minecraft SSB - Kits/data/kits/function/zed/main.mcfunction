...
execute as @s[scores={kits.criterion.attack=1..}] if score @s kits.health4 <= @s kits.health3 as @e[limit=1,sort=nearest,tag=!Zed,tag=!InLabyrinth,nbt={HurtTime:10s}] at @s run function kits:zed/passivedamage

# Shurikens
scoreboard players add @s[scores={kits.ability1CD=..99}] kits.ability1CD 1
execute as @s[scores={kits.criterion.COS=1..},predicate=kits:items/zed/shurikens] as @s[scores={kits.ability1CD=100..}] at @s run function kits:zed/weapon1cos
...
execute as @e[type=area_effect_cloud,tag=zedshuriken] at @s run function kits:zed/shurikendamage

# Shadow Slash
scoreboard players add @s[scores={kits.ability2CD=..49}] kits.ability2CD 1
execute as @s[gamemode=!spectator,scores={kits.criterion.shift=1..},predicate=kits:items/zed/shurikens] as @s[scores={kits.ability2CD=50..}] at @s run function kits:zed/weapon1shift
