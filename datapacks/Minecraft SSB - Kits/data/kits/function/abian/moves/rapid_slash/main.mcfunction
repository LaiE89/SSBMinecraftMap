...
# VFX
function kits:abian/moves/rapid_slash/vfx/main
...
# Collision
execute as @e[distance=..3,tag=!Abian,tag=!InLabyrinth,type=!#kits:non_entity,tag=!Invincible] at @s run function kits:abian/moves/rapid_slash/hit

# Stops if near RS end marker
execute if entity @e[distance=..3,limit=1,sort=nearest,tag=AbianRslashEndMarker,type=area_effect_cloud] run function kits:abian/moves/rapid_slash/reached_end
