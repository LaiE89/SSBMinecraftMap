...
execute as @s[scores={kits.raycastTick=2222..}] positioned ^ ^ ^ run execute as @e[distance=..2,limit=1,sort=nearest,type=!#kits:non_entity,tag=!Invincible,tag=!Ness,tag=!InLabyrinth] at @s run function kits:ness/smashknockback
execute as @s[scores={kits.raycastTick=..8}] positioned ^ ^ ^0.5 run function kits:ness/smash
