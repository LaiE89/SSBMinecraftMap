...
execute if entity @e[type=area_effect_cloud,tag=Dragonborn_VFX_ExpandingSphere] as @e[type=area_effect_cloud,tag=Dragonborn_VFX_ExpandingSphere] at @s run function kits:dragonborn/expanding_sphere/run

scoreboard players add @s[scores={kits.ability1CD=..99}] kits.ability1CD 1
execute if entity @s[scores={kits.criterion.COS=1..}] if entity @s[scores={kits.ability1CD=100..},predicate=kits:items/dragonborn/dawnbreaker] at @s run function kits:dragonborn/weapon1cos

scoreboard players add @s[scores={kits.ability2CD=..9}] kits.ability2CD 1
execute if entity @s[gamemode=!spectator,scores={kits.criterion.shift=1..}] if entity @s[scores={kits.ability2CD=10..},predicate=kits:items/dragonborn/dawnbreaker] at @s run function kits:dragonborn/weapon1shift

execute as @e[type=area_effect_cloud,tag=DragonbornForce] at @s run function kits:dragonborn/forceprojectile
execute as @e[tag=DragonbornMarked] at @s run function kits:dragonborn/markedeffects
