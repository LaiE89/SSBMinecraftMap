...
execute if entity @s[tag=RyzeFlux] as @e[type=!#kits:non_entity,tag=!RyzeCurTarget,tag=!RyzeFluxTarget,sort=nearest,distance=..10,tag=!Invincible,tag=!Ryze,tag=!InLabyrinth] at @s run function kits:ryze/fluxspread
