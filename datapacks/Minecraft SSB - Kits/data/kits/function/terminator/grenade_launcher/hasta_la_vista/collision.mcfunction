...
execute positioned ^ ^ ^5.5 as @e[distance=..7,tag=!Terminator,tag=!InLabyrinth,type=!#kits:non_entity,tag=!Invincible] at @s run function kits:terminator/grenade_launcher/hasta_la_vista/hit

# Plays spiral fire animation
summon area_effect_cloud ~ ~ ~ {Particle:{type:"block",block_state:"minecraft:air"},Radius:0f,WaitTime:0,Duration:15,Tags:["TerminatorHLVGrenadeFireAnimation"]}
