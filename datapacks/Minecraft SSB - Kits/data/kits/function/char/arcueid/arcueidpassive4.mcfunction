...
execute as @s[scores={kits.raycastTick=2222}] positioned ^ ^ ^ run execute as @e[distance=..2,limit=1,sort=nearest,type=!#kits:non_entity,tag=!Invincible,tag=!Arcueid,tag=!InLabyrinth] at @s run function kits:char/arcueid/chained
execute as @s[scores={kits.raycastTick=2222}] run scoreboard players set @s kits.ability4CD 0

execute as @s[scores={kits.raycastTick=..20}] positioned ^ ^ ^0.5 run function kits:char/arcueid/arcueidpassive4	
