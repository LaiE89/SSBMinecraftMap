...
summon area_effect_cloud ~ ~ ~ {Tags:["BradPunch1"],Particle:{type:"block",block_state:"minecraft:air"},Radius:0f,WaitTime:0,Duration:3}
...
execute as @e[distance=..4,limit=1,sort=nearest,type=!#kits:non_entity,tag=!Invincible,tag=!Brad,tag=!InLabyrinth] at @s run damage @s 4 bypass:player_attack by @e[type=area_effect_cloud,tag=BradPunch1,limit=1,sort=nearest] from @p[tag=Brad]
kill @e[type=area_effect_cloud,tag=BradPunch1]
