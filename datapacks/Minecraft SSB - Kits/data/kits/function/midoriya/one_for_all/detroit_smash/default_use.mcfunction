...
execute as @e[nbt={HurtTime:10s},distance=..5,limit=1,sort=nearest,tag=!Midoriya] at @s run function kits:midoriya/one_for_all/detroit_smash/hit

# Knocks hit enemy back
scoreboard players set @s kits.raycastTick 0
execute positioned ~ ~ ~ rotated ~ 0 run function kits:midoriya/one_for_all/detroit_smash/kb
