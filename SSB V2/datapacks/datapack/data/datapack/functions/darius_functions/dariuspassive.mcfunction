...
# Passive
execute at @p[tag=Darius,scores={Attack=1..}] run scoreboard players add @e[tag=!Darius,distance=..8,limit=1,sort=nearest,nbt={HurtTime:10s}] DariusPassive 1
execute if entity @e[tag=DariusNoxianMight] run tag @p[tag=Darius,tag=!DariusRage] add DariusRage

execute at @p[tag=Darius,tag=DariusRage,scores={Attack=1..}] run scoreboard players set @e[tag=!Darius,distance=..8,limit=1,sort=nearest,nbt={HurtTime:10s},scores={DariusPassive=..4}] DariusPassive 5
...
execute as @p[gamemode=!spectator,tag=Darius,scores={Shift=1..},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",tag:{Unbreakable:1,display:{Name:'[{"text":"Darius\' Axe","italic":false,"color":"red"}]'}}}}] as @p[tag=Darius,scores={Ability1CD=80..}] at @s run function datapack:darius_functions/weapon1cos
...
execute as @p[gamemode=!spectator,tag=Darius,scores={Ability2CD=150..},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",tag:{Unbreakable:1,display:{Name:'[{"text":"Darius\' Skills","italic":false,"color":"dark_red"}]'}}}}] as @p[tag=Darius,scores={COS=1..}] at @s run function datapack:darius_functions/weapon2cos

# Crippling Strike
scoreboard players add @e[tag=Darius,scores={Ability3CD=..69}] Ability3CD 1
execute as @p[tag=Darius,scores={Ability3CD=70..},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",tag:{Unbreakable:1,display:{Name:'[{"text":"Darius\' Axe","italic":false,"color":"red"}]'}}}}] as @p[tag=Darius,scores={COS=1..}] at @s run function datapack:darius_functions/weapon1shift

# Noxian Guillotine
scoreboard players set @p[tag=Darius,scores={Kills=1..}] Ability4CD 345
scoreboard players reset @p[tag=Darius,scores={Kills=1..}] Timer

scoreboard players add @e[tag=Darius,scores={Ability4CD=..349}] Ability4CD 1
execute as @p[tag=Darius,scores={Ability4CD=350..},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",tag:{Unbreakable:1,display:{Name:'[{"text":"Darius\' Skills","italic":false,"color":"dark_red"}]'}}}}] as @p[tag=Darius,scores={Shift=1..}] at @s run function datapack:darius_functions/weapon2shift
